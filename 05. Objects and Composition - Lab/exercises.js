...
function fromJSONToHTML(input) {
    ...
    const headerRow = `  <tr>${headers.map(h => `<th>${escapeHtml(h.trim())}</th>`).join('')}</tr>`;
    const rows = array
        .map(entry =>
            `  <tr>${headers.map(h => `<td>${escapeHtml(entry[h])}</td>`).join('')}</tr>`
        )
        .join('\n');
...
console.log(fromJSONToHTML(`[{"Name":"Stamat",
    "Score":5.5},
    {"Name":"Rumen",
    "Score":6}]`
  ));
